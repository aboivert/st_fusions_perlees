...
if fusions_file is not None:
    ...
    if analyse_stops:
            ...
            if stops.empty:
                ...
                for index, row in names_group.iterrows():
                    ...
                    if (temp_data.problem==1).any():
                        ...
                        st.slider("Rayon des points", min_value=100, max_value=300, value=3, step=1, key=value_key)
                        ...
                        view_state = pydeck.ViewState(
                            latitude=temp_data.stop_lat.mean(), longitude=temp_data.stop_lon.mean(), controller=True, zoom=10 , pitch=30
                        )
                        chart = pydeck.Deck(
        ...
        with st.expander("Analyse de distance entre zones d'arrêt"):
            ...
            st.slider("Radius 2", min_value=25, max_value=300, value=30, step=1, key="radius_2")
            point_layer = pydeck.Layer(
                "ScatterplotLayer",
                data=barycentre_df2,
                #id="capital-cities",
                ...
                pickable=True,
                auto_highlight=True,
                get_radius=st.session_state.radius_2,
            )
            view_state = pydeck.ViewState(
